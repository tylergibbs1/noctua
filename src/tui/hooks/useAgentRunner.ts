...
export function useAgentRunner(opts: {
  model?: string;
  ...
  const executeQuery = useCallback(async (queryText: string) => {
    ...
    try {
          ...
          updateItem(item => {
            ...
            for (let i = events.length - 1; i >= 0; i--) {
              ...
              if (
                ...
                events[i] = { ...de, completed: true, endEvent: event };
                break;
              }
            }
            return { ...item, events, activeToolId: undefined };
          });
          setWorkingState({ status: 'thinking' });
        },
      ...
      if (controller.signal.aborted) {
        ...
        if (result.sessionId) {
          (opts.sessionIdRef as React.MutableRefObject<string | undefined>).current = result.sessionId;
        }
        updateItem(item => ({
          ...item,
  ...
  }, [opts.model, opts.sessionIdRef, isProcessing]);
